...
const ShortAd = ({ ad, isActive, isScrolling }: { ad: ShortsAd, isActive: boolean, isScrolling: boolean }) => {
...
const ShortVideo = ({ short, isActive, onCommentsToggle, isScrolling }: { short: Short, isActive: boolean, onCommentsToggle: (isOpen: boolean) => void, isScrolling: boolean }) => {
...
export default function Shorts() {
  ...
  const { shorts, isInitialHomeLoading, setIsFullScreen } = useVideoStats();
  ...
  useEffect(() => {
    if (!isInitialHomeLoading) {
      // Small delay after context data is ready before we stop the "loading" UI
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden flex flex-col">
      ...
      {!isAnyCommentOpen && (
        <div className="fixed top-0 left-0 right-0 z-[100] px-4 pt-6 pb-2 flex items-center justify-center gap-3 pointer-events-none transition-opacity duration-300">
          <div 
            onClick={(e) => {
              e.stopPropagation();
              setIsSearchOpen(true);
            }}
            className="w-full max-w-lg bg-white/5 backdrop-blur-2xl border border-white/10 rounded-full px-5 py-3 flex items-center gap-3.5 shadow-[0_10px_30px_rgba(0,0,0,0.5)] pointer-events-auto group hover:bg-white/10 transition-all cursor-pointer active:scale-[0.98]"
          >
            {shortsSearchQuery && isGridView === false ? (
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  setIsGridView(true);
            ...
            <span className="text-sm md:text-base text-white/90 flex-1 font-semibold tracking-tight truncate">
              {shortsSearchQuery || "Search Shorts"}
            </span>
            {shortsSearchQuery && (
              <button
                onClick={(e) => {
          ...
          <Link 
            to="/live" 
            state={{ from: "/shorts" }}
            className="flex items-center justify-center group pointer-events-auto shrink-0 relative"
            title="Live"
            id="shorts-live-btn"
          >
            <div className="px-4 py-2 bg-white/10 backdrop-blur-2xl border border-white/10 rounded-full flex items-center gap-2 justify-center shadow-2xl group-hover:bg-white/20 transition-all active:scale-95 group-hover:border-red-500/30">
              <div className="relative">
                <div className="w-1.5 h-1.5 bg-red-500 rounded-full shadow-[0_0_10px_rgba(239,68,68,0.8)]" />
              </div>
              <span className="text-[10px] font-black text-white tracking-[0.15em] leading-none uppercase">LIVE</span>
              {liveShortsCount > 0 && (
                 <span className="bg-[#FF0050] text-[8px] font-black px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-lg border border-white/10">
                   {liveShortsCount}
                 </span>
              )}
            </div>
