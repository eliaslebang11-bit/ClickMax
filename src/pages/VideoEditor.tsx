...
export default function VideoEditor() {
  ...
  const handlePost = async () => {
    ...
    try {
      let fileToUpload = videoData.file;

      // Convert a blob Url of recorded video to file if file object is missing
      if (!fileToUpload && videoData.videoUrl && videoData.videoUrl.startsWith('blob:')) {
        console.log("[EDITOR] Converting video blob URL to File object...");
        const response = await fetch(videoData.videoUrl);
        const blob = await response.blob();
        fileToUpload = new File([blob], `recorded-short-${Date.now()}.webm`, { type: 'video/webm' });
      }

      if (!fileToUpload) {
        // Fallback for mock preview
        console.warn("[EDITOR] No file to upload, generating mock dummy video file for sandbox environment");
        const fetchResponse = await fetch("https://storage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4");
        const blob = await fetchResponse.blob();
        fileToUpload = new File([blob], `mock-short-${Date.now()}.mp4`, { type: 'video/mp4' });
      }

      await startUpload({
        title: caption || "My Short Video",
        description: caption || "",
        type: "Short",
        videoFile: fileToUpload,
        thumbnailFile: null,
        thumbnailBlobUrl: videoData.thumbnail || "https://picsum.photos/seed/thumb/800/450",
        duration: videoData.duration || "0:15",
        username: user.username || "Guest",
        avatar: user.avatar || ""
      });

      navigate('/me', { state: { posted: true, type: "Shorts" } });
    } catch (error: any) {
      console.error("[EDITOR] Preparation and upload initiation failed:", error);
