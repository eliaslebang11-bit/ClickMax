...
import { ArrowLeft, Zap, Play, Check, Link as LinkIcon, Calendar, MoreHorizontal, MoreVertical, User } from "lucide-react";
...
export default function Profile() {
  ...
  const { stats, toggleFollow, profiles, homeVideos, shorts } = useVideoStats();
  ...
  if (!profile) {
    return (
      <div className="flex flex-col items-center justify-center h-full space-y-4">
        ...
        <button 
          onClick={() => navigate(-1)}
          className="px-6 py-2 bg-brand-surface border border-brand-border rounded-full font-bold"
        >
          Go Back
        </button>
      </div>
  ...
  const userVideos = homeVideos.filter(v => v.channelName === username && !v.isLive);
  const userStreamed = homeVideos.filter(v => v.channelName === username && v.isLive);
  const userShorts = shorts.filter(s => s.creator === username);
  const isFollowing = stats[username || '']?.isFollowing;
