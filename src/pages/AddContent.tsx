...
function VideoPreviewOverlay({ 
  ...
  useEffect(() => {
    ...
    // Prompt browser to play (fallback to muted if autoplay blocked)
    v.play().catch((err) => {
      console.log("Autoplay blocked, attempting muted playback:", err);
      v.muted = true;
      v.play().catch((e) => console.warn("Playback failed completely:", e));
    });
...
function PostDetailsOverlay({ 
  ...
  const [uploadStep, setUploadStep] = useState<"idle" | "uploading_video" | "uploading_thumbnail" | "saving">("idle");
  const thumbInputRef = useRef<HTMLInputElement>(null);

  const { user, session } = useUser();
  const { startUpload } = useVideoStats();
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      let fileToUpload: File | null = video.file || null;
      
      // Fetch blob into memory quickly as a File if needed
      if (!fileToUpload && video.videoUrl.startsWith('blob:')) {
        console.log("[POST] Quick fetching video blob...");
        const response = await fetch(video.videoUrl);
        const blob = await response.blob();
        fileToUpload = new File([blob], `${isShort ? 'short' : 'video'}-${Date.now()}.mp4`, { type: 'video/mp4' });
      }

      if (!fileToUpload) {
        throw new Error("No video file selected or available for upload.");
      }

      startUpload({
        title: title || (isShort ? "Uploading Short..." : "Untitled Video"),
        description: description,
        type: isShort ? "Short" : "Video",
        videoFile: fileToUpload,
        thumbnailFile: thumbnailFile,
        thumbnailBlobUrl: thumbnail,
      ...
      console.error("[POST] Initiating background upload failed:", error);
      alert("Preparing upload failed: " + (error?.message || String(error)));
      setIsPosting(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-8">
        ...
        <div className="flex gap-4 items-start">
          <div className="space-y-2">
            ...
            >
              {((thumbnailFile !== null) || video.isImage || (typeof thumbnail === 'string' && (thumbnail.startsWith('data:image') || thumbnail.startsWith('blob:') || thumbnail.includes('thumbnail') || thumbnail.match(/\.(jpeg|jpg|gif|png|webp)/i)))) ? (
                <img 
                  src={thumbnail} 
                  alt="Thumbnail" 
                ...
                <div className="w-full h-full bg-neutral-900 flex items-center justify-center">
                  {thumbnail ? (
                    ...
                    <span className="text-xs text-white/30 uppercase tracking-widest font-black">Cover</span>
                  )}
                </div>
              )}
      ...
      {/* Immersive Upload Progress Overlay */}
      <AnimatePresence>
        {uploadStep !== "idle" && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-black/95 z-[150] flex flex-col items-center justify-center p-6 text-center"
          >
            <div className="relative w-40 h-40 mb-8">
              <svg className="w-full h-full" viewBox="0 0 100 100">
                ...
                <circle 
                  ...
                  strokeDashoffset={
                    263.89 - (263.89 * (
                      uploadStep === "uploading_video" ? videoProgress :
                      uploadStep === "uploading_thumbnail" ? thumbProgress :
                      uploadStep === "saving" ? 100 : 0
                    )) / 100
                  }
                  strokeLinecap="round" 
                  fill="transparent" 
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-black">
                  {uploadStep === "uploading_video" ? `${Math.round(videoProgress)}%` :
                   uploadStep === "uploading_thumbnail" ? `${Math.round(thumbProgress)}%` :
                   uploadStep === "saving" ? "99%" : "0%"}
                </span>
                <span className="text-[10px] font-black uppercase text-white/40 tracking-widest mt-1">
                  Progress
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3 max-w-xs">
              <h3 className="text-2xl font-black uppercase tracking-tight italic">
                {uploadStep === "uploading_video" && "Uploading Video..."}
                {uploadStep === "uploading_thumbnail" && "Uploading Cover..."}
                {uploadStep === "saving" && "Publishing Post..."}
              </h3>
              <p className="text-sm font-bold text-white/50 leading-relaxed">
                {uploadStep === "uploading_video" && "Transferring high quality stream chunks securely."}
                {uploadStep === "uploading_thumbnail" && "Uploading high definition thumbnail assets."}
                {uploadStep === "saving" && "Configuring stream metadata and saving your media."}
              </p>
            </div>

            {/* Minor Activity Indicator */}
            <div className="mt-12 flex items-center gap-2.5 bg-white/5 border border-white/10 px-4 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest text-white/60">
              <RefreshCw className="w-4 h-4 animate-spin text-blue-500" />
              <span>Optimizing CDN Playback</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export default function AddContent() {
  ...
  const startRecording = () => {
    ...
    mediaRecorder.onstop = () => {
      ...
      const file = new File([blob], `recorded-short-${Date.now()}.webm`, { type: "video/webm" });
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      const newVids = await Promise.all(Array.from(files).map(async (file, idx) => {
        ...
        let capturedThumbnailUrl = url;

        if (!isImage) {
          const frameResult = await new Promise<{ duration: string; thumbnail: string }>((resolve) => {
            ...
            const timeoutId = setTimeout(() => {
              if (!resolved) {
                resolved = true;
                resolve({ duration: "0:00", thumbnail: url });
              }
            }, 3000);
          ...
          durationStr = frameResult.duration;
          capturedThumbnailUrl = frameResult.thumbnail;
        }

        return {
          id: `local-${Date.now()}-${idx}`,
          thumbnail: capturedThumbnailUrl,
          videoUrl: url,
      ...
      if (newVids.length > 0) {
        setSelectedGalleryVideo(newVids[0]);
      }
    }
  };
    ...
    <div className="fixed inset-0 bg-black z-[100] flex flex-col text-white animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className={cn(
        ...
        <button 
          ...
          className="p-1.5 px-3 hover:bg-white/10 rounded-full transition-colors group flex items-center gap-1.5 text-white border border-white/15"
          title="Cancel and close"
        >
          <X className="w-5 h-5 group-hover:rotate-90 transition-transform duration-300" />
          <span className="text-xs font-black uppercase tracking-widest text-white/80">Cancel</span>
        </button>
      ...
      <div className="flex-1 overflow-y-auto relative">
        ...
        <AnimatePresence>
          ...
          {showPostDetails && selectedGalleryVideo && (
            <PostDetailsOverlay 
              key="post-details-overlay"
              video={selectedGalleryVideo}
              onClose={() => setShowPostDetails(false)}
              onPost={() => navigate('/me', { state: { posted: true, type: selectedGalleryVideo.type === "Short" ? "Shorts" : "Videos" } })}
            />
          )}
        </AnimatePresence>
      </div>
