...
export const VideoUpload: React.FC = () => {
  ...
  const startUpload = async (fileToUpload: File, videoTitle: string) => {
    ...
    setIsUploading(true);
    setUploadPercent(0);
    setProgress({ status: 'uploading', message: 'Analyzing and re-encoding video...' });

    try {
      // 1. Automatically compress the video first
      let processedFile = fileToUpload;
      try {
        processedFile = await compressVideo(fileToUpload, (compPct) => {
          setUploadPercent(compPct);
          setProgress({
            status: 'uploading',
            message: `Compressing & Re-encoding: ${compPct}%`
          });
        });
      } catch (comError) {
        console.warn('[UPLOAD-UI] Compression fallback triggered:', comError);
      }

      setUploadPercent(0);
      setProgress({ status: 'uploading', message: 'Initializing storage transfer...' });
      ...
      const uploadedUrl = await supabaseService.uploadFile(
        ...
        (percent) => {
          ...
          if (percent >= 100) {
            ...
            setProgress({ 
              status: 'uploading', 
              message: `Uploading to Supabase: ${percent}%` 
            });
          }
        }
      );
      
      if (!uploadedUrl) throw new Error('Failed to upload video to Supabase Storage');
      ...
      const success = await supabaseService.insertVideo({
        ...
        channelName: user?.username || session?.user?.email?.split('@')[0] || "Guest",
      ...
      if (!success) throw new Error("Database update failed after upload");
      ...
      console.error('[UPLOAD-UI] Detailed failure:', error);
      setIsUploading(false);
      setProgress({ status: 'error', message: 'Upload failed. Please try again.' });
    }
  };
