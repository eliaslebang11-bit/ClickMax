...
import AmbientWebGLBackground from "./AmbientWebGLBackground";
...
export default function Layout() {
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col bg-transparent overflow-hidden relative">
      <AmbientWebGLBackground />
      <div className="flex flex-1 min-h-0 overflow-hidden relative z-10 bg-transparent">
        {/* Sidebar - Desktop Only */}
        {!isFullScreen && (
          <aside 
            className={cn(
              "hidden md:block bg-brand-bg/40 backdrop-blur-md border-r border-brand-border pt-6 transition-all duration-300",
        ...
        {/* Main Content */}
        <main className={cn("flex-1 min-w-0 overflow-hidden relative", (isAddContentPage || location.pathname === "/search") ? "pb-0" : "pb-14 md:pb-0")}>
          <Outlet context={{ 
            setIsSidebarOpen, 
            isSidebarOpen, 
            searchQuery, 
      ...
      {!isFullScreen && !isAddContentPage && location.pathname !== "/search" && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-14 bg-brand-bg/60 backdrop-blur-md border-t border-brand-border flex items-center justify-around px-2 z-50">
          ...
          <BottomNavItem icon={Bell} label="Activity" to="/activity" active={location.pathname === "/activity"} />
