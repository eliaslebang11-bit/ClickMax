...
export default function CreateModal({ isOpen, onClose }: CreateModalProps) {
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      if (file) {
        // Real upload to Cloudflare Stream
        const streamData = await supabaseService.uploadToCloudflareStream(file, (p) => {
          setProgress(p);
        });
        
        if (!streamData) throw new Error("Upload failed to store file. Please check Cloudflare settings.");
        finalVideoUrl = streamData.videoUrl;
        
        if (!finalThumbnail) {
          finalThumbnail = `https://videodelivery.net/${streamData.uid}/thumbnails/thumbnail.jpg?time=2s&height=720`;
        }
      ...
      const message = error.hint ? `${error.message}. ${error.hint}` : (error.message || "Upload failed. Please check your internet and login status.");
      alert(message);
      setStep('details');
      setUploading(false);
    }
            ...
            {step === 'uploading' && (
              ...
              >
                ...
                <div className="text-center space-y-2">
                  <h2 className="text-2xl font-bold">Uploading your video...</h2>
                  <p className="text-sm text-brand-muted">Please keep this window open while we process your content.</p>
                </div>
                <div className="flex items-center gap-2 text-xs text-brand-muted font-bold">
                  <Loader2 className="w-3 h-3 animate-spin" />
                  <span>Optimizing for 4K Playback</span>
                </div>
              </motion.div>
            )}
