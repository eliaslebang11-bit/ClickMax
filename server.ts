...
const upload = multer({
  storage: storage,
  limits: {
    fileSize: 50 * 1024 * 1024 * 1024, // 50GB practically unlimited
  },
});
...
const supabaseUrl = process.env.VITE_SUPABASE_URL || process.env.SUPABASE_URL || "https://fbjdiqrfvwvzbyzczuqp.supabase.co";
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZiamRpcXJmdnd2emJ5emN6dXFwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg3NjM0NDQsImV4cCI6MjA5NDMzOTQ0NH0.u41w3VYnrGQjPGiOHF-eazzv6J-kCZUrSsBUS_-8tc";
...
async function ensureStorageBucket() {
  ...
  try {
    ...
    if (error || !data) {
      console.log("[STORAGE] Bucket 'videos' not found, creating it...");
      const { error: createErr } = await supabaseAdmin.storage.createBucket('videos', {
        public: true,
      });
      if (createErr) {
        console.warn("[STORAGE] Note creating 'videos' bucket returned error:", createErr.message);
      } else {
        console.log("✅ [STORAGE] Bucket 'videos' created successfully");
      }
    } else {
      console.log("✅ [STORAGE] Checked and found bucket 'videos' exists");
    }
  } catch (err: any) {
    console.error("[STORAGE] Error checking/creating 'videos' storage bucket:", err.message);
  }
}
...
try {
  if (supabaseUrl && supabaseServiceKey) {
    ...
    ensureStorageBucket().catch(e => console.error("[STORAGE] Bucket initialization async failure:", e));
  } else {
...
// --- Database Fallback Helpers for missing Supabase Tables ---
const uploadsDir = path.join(process.cwd(), "uploads");
if (!fs.existsSync(uploadsDir)) {
  fs.mkdirSync(uploadsDir, { recursive: true });
}

const NOTIFICATIONS_FILE = path.join(uploadsDir, "local_notifications.json");
const LIKES_FILE = path.join(uploadsDir, "local_likes.json");
...
function readJsonFile(filePath: string, defaultVal: any) {
  try {
    ...
    console.error(`[DB-FALLBACK] Error reading ${filePath}:`, err);
  }
  return defaultVal;
}
...
function writeJsonFile(filePath: string, data: any) {
  try {
    ...
    console.error(`[DB-FALLBACK] Error writing ${filePath}:`, err);
  }
}
...
async function createNotificationHelper(notif: {
  recipient_id: string;
  actor_id: string;
  ...
  try {
    if (!supabaseAdmin) throw new Error("No supabase admin client");
    
    // Pre-validate UUIDs to prevent db driver crashes and relation error logs
    const isRecipientValidObj = isValidUuid(notif.recipient_id);
    const isActorValidObj = isValidUuid(notif.actor_id);
    const isVideoValidObj = !notif.video_id || isValidUuid(notif.video_id);
    const isShortValidObj = !notif.short_id || isValidUuid(notif.short_id);
    const isCommentValidObj = !notif.comment_id || isValidUuid(notif.comment_id);

    if (!isRecipientValidObj || !isActorValidObj || !isVideoValidObj || !isShortValidObj || !isCommentValidObj) {
      throw new Error("Bypassing database insert due to local/mock identifiers");
    }

    const { error } = await supabaseAdmin.from("notifications").insert([notif]);
    if (!error) return;
    if (error.message?.includes("relation") || error.message?.includes("find the table") || error.code === 'PGRST205') {
       throw error;
    }
  } catch (err) {
    console.log("[Notifications] Saving notification to local store.");
    ...
    const newNotif = {
      ...
      video_id: notif.video_id || null,
      short_id: notif.short_id || null,
...
const authenticate = async (req: Request, res: Response, next: NextFunction) => {
  ...
  // Robust client-side token decoding fallback (for offline, local mock, or connection issues)
  try {
    if (token) {
    ...
    console.warn("[AUTH] JWT decode fallback failed:", err?.message || err);
  }
...
async function ensureProfile(user: any) {
  ...
  try {
    ...
    if (!profile) {
      let baseUsername = user.email ? user.email.split('@')[0] : `user_${user.id.slice(0, 5)}`;
      // Sanitize username
      baseUsername = baseUsername.replace(/[^a-zA-Z0-9_]/g, '');
      if (!baseUsername) baseUsername = `user_${user.id.slice(0, 5)}`;
      ...
      while (attempts < 5) {
        ...
        if (error && !errMsg.includes("unique") && !errMsg.includes("duplicate")) {
          console.error("[PROFILE] Database error creating profile:", error);
          break;
        }
        attempts++;
      }
      ...
      if (!createdProfile) {
        const fallbackUsername = `user_${user.id.replace(/[^a-zA-Z0-9_]/g, '').slice(0, 15)}`;
        const fallbackHandle = `@${fallbackUsername.toLowerCase()}`;
        const { data: finalProfile, error: finalErr } = await supabaseAdmin
          .from("profiles")
          .upsert([{
            id: user.id,
        ...
        if (finalErr) {
          console.error("[PROFILE] Fatal: Ultimate profile fallback failed:", finalErr);
          return { username: fallbackUsername, avatar_url: "" };
        }
...
/**
 * Dynamic column detection to only insert columns that actually exist in the table.
 */
async function filterExistingColumns(tableName: string, payload: Record<string, any>): Promise<Record<string, any>> {
  if (!supabaseAdmin) {
    return payload;
  }
  ...
  while (attempts < 10) {
    ...
    const match1 = errMsg.match(/column "([^"]+)" does not exist/i);
    ...
    if (!missingCol) {
      for (const k of keys) {
        if (errMsg.toLowerCase().includes(k.toLowerCase())) {
          missingCol = k;
          break;
        }
      }
    ...
    if (missingCol && filtered[missingCol] !== undefined) {
      console.log(`[SCHEMA-DETECTOR] Pruning non-existent column: ${missingCol}`);
...
app.post("/api/upload", authenticate, upload.single("video"), async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[UPLOAD] Starting Supabase storage upload for user ${user.id}: ${fileName}`);
    ...
    try {
      ...
      console.warn(`[UPLOAD] Supabase storage upload failed (${sbError.message || sbError}). Falling back to local disk copy...`);
    ...
    console.log(`[UPLOAD] Supabase Storage upload success: ${publicUrl}`);
    ...
    const processedPayload = await filterExistingColumns("videos", rawPayload);
    ...
    // Explicitly add 'owner' property to the response to match frontend feed expectations
 ...
 * Alternative: Frontend uses this to upload files directly to R2 safely.
 */
app.post("/api/upload/url", authenticate, async (req, res) => {
  try {
    const { fileName, contentType, type, fileSize } = req.body; // type: 'video' | 'short' | 'profile-picture' | 'thumbnail' | 'banner' | 'image'
    const user = (req as any).user;
    ...
    if (!fileName || !contentType || !type) {
      return res.status(400).json({ error: "Missing required fields: fileName, contentType, type" });
    }

    // 1. Rigorous backend validation for file types and size constraints
    const cleanType = String(type).toLowerCase();
    const cleanContentType = String(contentType).toLowerCase();
    
    // Limits
    const MAX_VIDEO_SIZE = 500 * 1024 * 1024; // 500MB
    const MAX_IMAGE_SIZE = 10 * 1024 * 1024; // 10MB (profile pics max 5MB is checked below)
    const MAX_PROFILE_IMAGE_SIZE = 5 * 1024 * 1024; // 5MB

    if (cleanType === 'video' || cleanType === 'short') {
      if (!cleanContentType.startsWith('video/')) {
        return res.status(400).json({ error: "Invalid type: Video uploads must be video format files" });
      }
      if (fileSize && Number(fileSize) > MAX_VIDEO_SIZE) {
        return res.status(400).json({ error: "Video file exceeds maximum allowed size of 500MB" });
      }
    } else {
      if (!cleanContentType.startsWith('image/')) {
        return res.status(400).json({ error: "Invalid type: Picture uploads must be image format files" });
      }
      if (cleanType === 'profile-picture' || cleanType === 'avatar') {
        if (fileSize && Number(fileSize) > MAX_PROFILE_IMAGE_SIZE) {
          return res.status(400).json({ error: "Profile picture exceeds maximum allowed size of 5MB" });
        }
      } else if (fileSize && Number(fileSize) > MAX_IMAGE_SIZE) {
        return res.status(400).json({ error: "Image file exceeds maximum allowed size of 10MB" });
      }
    }

    // 2. Folder organization mapping: profile-pictures/, videos/, shorts/
    let folder = "others";
    if (cleanType === "profile-picture" || cleanType === "avatar") {
      ...
      folder = "shorts";
    } else if (cleanType === "banner") {
      folder = "banners";
    } else if (cleanType === "thumbnail" || cleanType === "image") {
      folder = "thumbnails";
    }

    const key = `${user.id}/${folder}/${Date.now()}-${fileName.replace(/[^a-zA-Z0-9.-]/g, "_")}`;

    console.log(`[PRESIGN] Generating URL for ${cleanType} in folder ${folder}: ${key}`);
    
    console.warn("[PRESIGN] Bypassing R2 as requested. Instructing client to use proxy mode.");
    return res.json({ 
      uploadUrl: null, 
      publicUrl: null, 
      key, 
...
/**
 * 2.1 REGISTER: Register the file URL, storage key, type, format size with user metadata inside Supabase
 */
app.post("/api/upload/register", authenticate, async (req, res) => {
  try {
    const user = (req as any).user;
    if (!user) {
      return res.status(401).json({ error: "Authentication required to register uploads" });
    }

    const { fileUrl, key, fileType, fileSize, uploadType } = req.body;

    if (!fileUrl || !key || !fileType || !fileSize || !uploadType) {
      return res.status(400).json({ 
        error: "Missing registering attributes: fileUrl, key, fileType, fileSize, uploadType are required" 
      });
    }

    // 1. Size Constraints & Format sanity
    const size = Number(fileSize);
    const maxVideoSize = 500 * 1024 * 1024;
    const maxImageSize = 10 * 1024 * 1024;
    const maxProfileImageSize = 5 * 1024 * 1024;

    if (uploadType === 'video' || uploadType === 'short') {
      if (!fileType.startsWith('video/')) {
        return res.status(400).json({ error: "Invalid registration: Cannot register video for non-video mimetype" });
      }
      if (size > maxVideoSize) {
        return res.status(400).json({ error: "Registration failed: Video exceeds 500MB limit" });
      }
    } else {
      if (!fileType.startsWith('image/')) {
        return res.status(400).json({ error: "Invalid registration: Cannot register image for non-image mimetype" });
      }
      if (uploadType === 'profile-picture' || uploadType === 'avatar') {
        if (size > maxProfileImageSize) {
          return res.status(400).json({ error: "Registration failed: Profile picture exceeds 5MB limit" });
        }
      } else if (size > maxImageSize) {
        return res.status(400).json({ error: "Registration failed: Image/Banner exceeds 10MB limit" });
      }
    }
    ...
    // 3. Keep database consistent: insert meta row inside public.uploads
    console.log(`[REGISTER] Inserting upload record for user ${user.id}: ${key}`);
    ...
    if (dbError) {
      console.warn("[REGISTER] Supabase insert warning (non-blocking): Unable to log to uploads table, but proceeding anyway:", dbError.message);
    }

    // 4. Standalone entity automappings (for user profiles)
    if (uploadType === 'profile-picture' || uploadType === 'avatar') {
      console.log(`[REGISTER] Automapping avatar url to user ${user.id}: ${fileUrl}`);
      ...
      if (profileErr) {
        console.warn(`[REGISTER] Profile avatar automap failed: ${profileErr.message}`);
      }
    } else if (uploadType === 'banner') {
      console.log(`[REGISTER] Automapping banner url to user ${user.id}: ${fileUrl}`);
      const { error: profileErr } = await supabaseAdmin
        .from("profiles")
        .update({ banner_url: fileUrl, updated_at: new Date().toISOString() })
        .eq("id", user.id);
      
      if (profileErr) {
        console.warn(`[REGISTER] Profile banner automap failed: ${profileErr.message}`);
      }
    }

    return res.status(201).json({
      success: true,
      message: "R2 upload registered in Supabase metadata store.",
      record
    });
  } catch (error: any) {
    console.error("[REGISTER] System Registration failed:", error);
    res.status(500).json({ error: "Upload Registration failed", details: error.message });
  }
});

/**
 * 2.2 CLEANUP: Remove files if uploads or video creation fail, keeping database consistent
 */
app.post("/api/upload/cleanup", authenticate, async (req, res) => {
  try {
    const { key, fileUrl } = req.body;
    const user = (req as any).user;

    if (!user) {
      return res.status(401).json({ error: "Authentication required for upload cleanup" });
    }

    if (!key) {
      return res.status(400).json({ error: "R2 storage key is required for cleanup" });
    }

    console.log(`[CLEANUP] Request for R2 object ${key} by user ${user.id}`);

    // Ensure the key belongs to the current user (security requirement)
    if (!key.startsWith(`${user.id}/`)) {
      return res.status(403).json({ error: "Unauthorized cleanup attempt: Can only delete your own uploads" });
    }

    // 1. Delete from Cloudflare R2
    await storageService.deleteFile(key);

    // 2. Delete and clean up from Supabase uploads if they exist
    ...
    if (dbError) {
      console.warn(`[CLEANUP] Supabase uploads log deletion warning: ${dbError.message}`);
    }

    res.json({ success: true, message: "Storage cleanup action completed successfully" });
  } catch (error: any) {
    console.error("[CLEANUP] Error during rollback execution:", error);
    res.status(500).json({ error: "Failed to perform cleanup execution", details: error.message });
  }
});
...
app.post("/api/upload/stream", authenticate, async (req, res) => {
  try {
    ...
    const result = await cloudflareStreamService.generateDirectUploadUrl({
      fileName: fileName || "Untitled Video",
      creatorId: user.id
    });
    ...
    console.error("[STREAM-UPLOAD] Fatal error generating direct upload URL:", error);
    res.status(500).json({ error: error.message || "Failed to generate Cloudflare Stream upload link" });
  }
});
...
app.post("/api/upload/proxy", authenticate, upload.single('file'), async (req, res) => {
  try {
    ...
    console.log(`[PROXY-UPLOAD] Phase 1: Intake Complete. Size: ${(file.size / 1024 / 1024).toFixed(2)} MB. Start Supabase storage upload...`);
    ...
    try {
      ...
      console.warn(`[PROXY-UPLOAD] Supabase storage upload failed (${sbError.message || sbError}). Falling back to local disk copy...`);
      const safeFileName = key.replace(/\//g, '_');
      const uploadsDir = path.join(process.cwd(), 'public', 'uploads');
      if (!fs.existsSync(uploadsDir)) fs.mkdirSync(uploadsDir, { recursive: true });
      const destPath = path.join(uploadsDir, safeFileName);
    ...
    console.log(`[PROXY-UPLOAD] Phase 2: Upload Complete: ${publicUrl}`);
  ...
  try {
    ...
    res.status(500).json({
      ...
      meta: {
        ...
        title: req.body.description || "Short Video",
        thumbnail_url: req.body.avatar || "",
        created_at: new Date().toISOString()
      }
    });
  }
...
/**
 * 2.1.2 SHORTS: Delete Short Record
 */
app.delete("/api/shorts/:id", authenticate, async (req, res) => {
  const { id } = req.params;
  const user = (req as any).user;
  if (!user) return res.status(401).json({ error: "Authentication required" });

  try {
    const { data: short, error: fetchErr } = await supabaseAdmin
      .from("shorts")
      .select("*")
      .eq("id", id)
      .single();
    ...
    if (fetchErr) {
      console.error("Error fetching short to delete:", fetchErr);
      return res.status(404).json({ error: "Short not found" });
    }

    if (short.owner_id !== user.id) {
      return res.status(403).json({ error: "Unauthorized operation" });
    }

    // Cascade delete any referencing records in database to avoid foreign key violation
    try {
      await supabaseAdmin.from("comments").delete().eq("short_id", id);
    } catch (err) {
      console.warn("[CASZ] Non-critical shorts comments clear failed:", err);
    }
    try {
      await supabaseAdmin.from("likes").delete().eq("short_id", id);
    } catch (err) {
      console.warn("[CASZ] Non-critical shorts likes clear failed:", err);
    }
  ...
  try {
    ...
    res.status(500).json({
      ...
      meta: {
        ...
        thumbnail_url: req.body.thumbnail || req.body.thumbnailUrl,
        created_at: new Date().toISOString()
      }
    });
  }
...
/**
 * 2.2.2 VIDEOS: Delete Video Record
 */
app.delete("/api/videos/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (fetchErr) {
      console.error("Error fetching video to delete:", fetchErr);
    ...
    // Cascade delete any referencing records in database to avoid foreign key violation
    try {
      await supabaseAdmin.from("comments").delete().eq("video_id", id);
    } catch (err) {
      console.warn("[CASZ] Non-critical video comments clear failed:", err);
    }
    try {
      await supabaseAdmin.from("video_likes").delete().eq("video_id", id);
    } catch (err) {
      console.warn("[CASZ] Non-critical video_likes clear failed:", err);
    }
    try {
      await supabaseAdmin.from("likes").delete().eq("video_id", id);
    } catch (err) {
      console.warn("[CASZ] Non-critical video likes clear failed:", err);
    }
    try {
      await supabaseAdmin.from("watch_history").delete().eq("video_id", id);
    } catch (err) {
      console.warn("[CASZ] Non-critical video watch_history clear failed:", err);
    }
...
app.get("/api/videos/feed", async (req, res) => {
  ...
  try {
    ...
    // Fetch up to 120 most viewed ready videos to get a high-quality candidate pool
    ...
    // Custom randomized sort favoriting higher views (highest views to lowest with dynamic jitter)
    const candidates = data || [];
    const randomized = candidates
      .map((item: any) => {
        const v = Number(item.views_count ?? item.views ?? 0);
        const score = (v + 5) * (0.6 + Math.random() * 0.8);
        return { item, score };
      })
      .sort((a, b) => b.score - a.score)
      .map(x => x.item);

    // Paginate from the randomized candidates pool
    const paginated = randomized.slice(from, to + 1);
    res.json(paginated);
  } catch (error: any) {
...
app.get("/api/shorts/feed", async (req, res) => {
  ...
  try {
    ...
    // Fetch up to 120 most viewed shorts to get a high-quality candidate pool
    let { data, error } = await supabaseAdmin
      .from("shorts")
      .select("*, owner:profiles(username, handle, avatar_url)")
      .order("views_count", { ascending: false })
...
app.post("/api/videos/:id/like", authenticate, async (req, res) => {
  ...
  try {
    ...
    // 1. Try querying Supabase video_likes
    try {
      if (!supabaseAdmin || !isValidUuid(user.id) || !isValidUuid(id)) throw new Error("Bypassing database fetch due to local/mock user or video ID format");
      ...
      fallbackUsed = true;
      console.log("[Likes] Local JSON fallback utilized.");
      const likes = readJsonFile(LIKES_FILE, []);
      existing = likes.find((l: any) => l.user_id === user.id && l.video_id === id);
    }
    ...
    if (existing) {
      ...
      if (fallbackUsed) {
        let likes = readJsonFile(LIKES_FILE, []);
        likes = likes.filter((l: any) => !(l.user_id === user.id && l.video_id === id));
        writeJsonFile(LIKES_FILE, likes);
      }

      // Decrement like count in videos
      try {
        const { data: v } = await supabaseAdmin.from("videos").select("likes_count").eq("id", id).single();
        const currentLikes = v?.likes_count || 0;
        await supabaseAdmin.from("videos").update({ likes_count: Math.max(0, currentLikes - 1) }).eq("id", id);
      } catch (countErr) {
        console.warn("[LIKE] Decrement like count failed:", countErr);
      }
      ...
      // Increment like count in videos
      try {
        const { data: v } = await supabaseAdmin.from("videos").select("likes_count").eq("id", id).single();
        const currentLikes = v?.likes_count || 0;
        await supabaseAdmin.from("videos").update({ likes_count: currentLikes + 1 }).eq("id", id);
      } catch (countErr) {
        console.warn("[LIKE] Increment like count failed:", countErr);
      }
...
app.put("/api/profiles/me", authenticate, async (req, res) => {
  ...
  const { username, handle, bio, avatarUrl, bannerUrl } = req.body;
  ...
  try {
    ...
    // 3. Filter payload dynamically matching table columns
    const filteredPayload = await filterExistingColumns("profiles", updatePayload);

    // 4. Perform update and request updated record safely
    const { data, error } = await supabaseAdmin
      .from("profiles")
    ...
    if (!data) {
      // Fallback: fetch current profile record
      ...
      return res.json(fallbackProfile || { id: user.id });
    }

    res.json(data);
  } catch (error: any) {
    console.error("[PROFILE] Put API update failed:", error);
    res.status(500).json({ error: error.message });
  }
});
...
app.post("/api/profiles/:id/subscribe", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      ...
      // Decrement followers_count in profiles
      try {
        const { data: prof } = await supabaseAdmin
          ...
          .eq("id", channelId)
          .single();
        const currentCount = prof?.followers_count || 0;
        await supabaseAdmin
          .from("profiles")
          .update({ followers_count: Math.max(0, currentCount - 1) })
          .eq("id", channelId);
      } catch (countErr) {
        console.warn("[SUBSCRIBE] Decrement follower count failed:", countErr);
      }
      ...
      // Increment followers_count in profiles
      try {
        const { data: prof } = await supabaseAdmin
          .from("profiles")
          .select("followers_count")
        ...
        console.warn("[SUBSCRIBE] Increment follower count failed:", countErr);
      }
...
app.get("/api/notifications", authenticate, async (req, res) => {
  try {
    ...
    // Fetch notifications where recipient_id = user.id
    try {
      if (!supabaseAdmin || !isValidUuid(user.id)) throw new Error("Bypassing database fetch due to local/mock user ID format");
      ...
      fallbackUsed = true;
      console.log("[Notifications] Fallback fetch utilized.");
      const allNotifications = readJsonFile(NOTIFICATIONS_FILE, []);
      notificationsData = allNotifications
        .filter((n: any) => n.recipient_id === user.id)
    ...
    // Extract unique actor, video and short IDs
    const actorIds = [...new Set(notifications.map(n => n.actor_id).filter(Boolean))];
    const videoIds = [...new Set(notifications.map(n => n.video_id).filter(Boolean))];
    const shortIds = [...new Set(notifications.map(n => n.short_id).filter(Boolean))];

    // Parallel fetch helpers
    const [profilesRes, videosRes, shortsRes] = await Promise.all([
      actorIds.length > 0 && supabaseAdmin
        ? supabaseAdmin.from("profiles").select("id, username, avatar_url").in("id", actorIds)
        : Promise.resolve({ data: [] }),
      videoIds.length > 0 && supabaseAdmin
        ? supabaseAdmin.from("videos").select("id, title, thumbnail").in("id", videoIds)
        : Promise.resolve({ data: [] }),
      shortIds.length > 0 && supabaseAdmin
        ? supabaseAdmin.from("shorts").select("id, title, thumbnail, thumbnail_url").in("id", shortIds)
        : Promise.resolve({ data: [] })
    ]);
    ...
    const enriched = notifications.map(n => {
      ...
      return {
        ...
        targetType: n.short_id ? "shorts" : "video",
...
app.post("/api/notifications/read-all", authenticate, async (req, res) => {
  try {
    ...
    let fallbackUsed = false;
    try {
      if (!supabaseAdmin || !isValidUuid(user.id)) throw new Error("Bypassing database write because user ID is not a valid UUID format");
      const { error } = await supabaseAdmin
        .from("notifications")
        .update({ is_read: true })
        .eq("recipient_id", user.id);
...
app.post("/api/notifications/:id/read", authenticate, async (req, res) => {
  try {
    ...
    let fallbackUsed = false;
    try {
      if (!supabaseAdmin || !isValidUuid(id) || !isValidUuid(user.id)) {
        throw new Error("Bypassing database write because user or notification ID is not a valid UUID format");
      }
      const { error } = await supabaseAdmin
        .from("notifications")
        .update({ is_read: true })
    ...
    if (fallbackUsed) {
      let notifications = readJsonFile(NOTIFICATIONS_FILE, []);
      notifications = notifications.map((n: any) => {
        if (n.id === id && n.recipient_id === user.id) {
          return { ...n, is_read: true };
        }
        return n;
      });
    ...
    console.error("[NOTIFICATIONS] Mark index read error:", err);
    res.status(500).json({ error: err.message });
  }
});
...
app.get("/api/comments", async (req, res) => {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      try {
        if (vid) {
          const { data: vObj } = await supabaseAdmin.from("videos").select("owner_id").eq("id", vid).maybeSingle();
          if (vObj && vObj.owner_id) isUserUploaded = true;
        } else if (sid) {
          const { data: sObj } = await supabaseAdmin.from("shorts").select("owner_id").eq("id", sid).maybeSingle();
          if (sObj && sObj.owner_id) isUserUploaded = true;
        }
      } catch (checkErr) {
        console.error("Error checking video/short upload origin:", checkErr);
      }
...
async function seedNewStatsAndComments() {
  ...
  console.log("ℹ️ [BACKGROUND-SEED] Skipping automatic video/shorts stats override, views/likes generator, and comment auto-seeding. Respecting original Supabase database values.");
  return;
...
async function startServer() {
  ...
  // Serve local uploaded files statically in both dev and production mode
